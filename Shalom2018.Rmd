...
data_change_sum <- data_sum %>% 
  ...
  mutate(lag_value = lag(value),
         ...
         value_change = sum(diff(sign(diff_value)) != 0, na.rm = T)) %>% 
  ungroup() %>% 
  select(id, n, measure, value_change) %>% 
  distinct() 
...
Note that `id = 5` did not fluctuate on the sum scores but shows changes on the  item by item level.
...
data_change_item <- data %>% 
  ...
  mutate(lag_value = lag(value),
         ...
         value_change = sum(diff(sign(diff_value)) != 0, na.rm = T)) %>% 
  ungroup() %>% 
  select(id, n, measure, item, value_change) %>% 
  distinct()
...
Depending on the definition and research question this could actually be desired.
Only of variability on an item by item level is of theoretical interest it would be important to consider an adaptation of the method proposed by @Shalom2018.

For example `id = 5` shows no variability in the sum scores but most variability in item scores compared to other participants.
One potential way to address this would be to include item by item changes in the formula instead of changes in sum scores as indicated by the index `i` in $\Delta_{s_i}$, see the adapted formula below:
